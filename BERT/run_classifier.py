...
# Use Transformers' default filenames for saved config/weights
        ...
        with open(input_file, encoding=encoding) as fin:
            for raw in fin:
                ...
                if not line:
                    continue
                if MR:
                    # IMDB/MR-style: label<space>text
                    if ' ' not in line:
                        continue
                    label, text = line.split(' ', 1)
                else:
                    # CSV-style: robustly split at the LAST comma so commas in text are preserved
                    try:
...
def main():
    ...
    num_warmup_steps = int(args.warmup_proportion * num_train_optimization_steps) if num_train_optimization_steps else 0
    base_optimizer = optimizer.optimizer if hasattr(optimizer, 'optimizer') else optimizer
    scheduler = get_linear_schedule_with_warmup(
        base_optimizer,
        num_warmup_steps=num_warmup_steps,
        num_training_steps=num_train_optimization_steps if num_train_optimization_steps else 0,
    )
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                outputs = model(input_ids=input_ids,
                                ...
                                labels=label_ids)
                loss = outputs[0] if isinstance(outputs, (tuple, list)) else (outputs.loss if hasattr(outputs, 'loss') else outputs)
                if n_gpu > 1:
    ...
    if args.do_train:
        ...
        # Reload the fine-tuned model from output directory
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                outputs = model(input_ids=input_ids,
                                ...
                                attention_mask=input_mask,
                                labels=label_ids)
                tmp_eval_loss = outputs[0] if isinstance(outputs, (tuple, list)) else (outputs.loss if hasattr(outputs, 'loss') else outputs)
                outputs_no_label = model(input_ids=input_ids,
                                         token_type_ids=segment_ids,
                                         attention_mask=input_mask)
                logits = outputs_no_label[0] if isinstance(outputs_no_label, (tuple, list)) else outputs_no_label.logits
